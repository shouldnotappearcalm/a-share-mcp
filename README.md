...
本次更新新增 **实时行情数据模块**，支持盘中即时数据获取和技术指标分析。

### 🆕 新增工具

| 工具 | 功能 | 说明 |
|------|------|------|
| `get_realtime_kline` | 实时K线数据 | 支持 1/5/15/30/60 分钟线及日线/周线/月线 |
| `get_technical_indicators` | 技术指标计算 | MACD, KDJ, RSI, BOLL, MA, ATR, DMI 等 12+ 指标 |
| `get_realtime_quote` | 实时行情快照 | 一键获取最新价格、涨跌幅等关键数据 |

### ✨ 功能亮点

- **盘中即时更新**: 交易时间内数据实时可用，无需等待 Baostock 延迟数据
- **多代码格式兼容**: 支持 sh600000、600000、sh.600000、600519.XSHG 等多种格式
- **技术指标集成**: 结合 MyTT 指标库，实时计算 12+ 种技术指标
- **双核心数据源**: 腾讯/新浪互为备份，确保数据稳定性

### 📝 其他改进

- 修复 JSON 格式输出时 Timestamp 序列化问题
- 新增 `tests/test_realtime_market.py` 测试文件（20 个测试用例）
